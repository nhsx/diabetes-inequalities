...
```python
sns.lmplot(data=g, y='DM-prevalance', x='IMD')
```

```python
sns.lmplot(data=g, x='IMD', y='proportion')
```
...
```python
a = DCdata.groupby(['IMD (q5)', 'Age_group']).size().reset_index().pivot(index='Age_group', columns='IMD (q5)')
```
