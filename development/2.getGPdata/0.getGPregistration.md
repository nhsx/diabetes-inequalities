...
if not os.path.exists(f'{name}-female-LSOA11.csv'):
    ...
    for sex in ['male', 'female']:
        shutil.move(f'source/gp-reg-pat-prac-lsoa-{sex}.csv', f'source/{name}-{sex}-LSOA11.csv')
```
...
for sex in ['male', 'female']:
    tmp = pd.read_csv(
        f'source/{name}-{sex}-LSOA11.csv', skiprows=1, usecols=cols, 
        dtype=dtype, names=dtype.keys())
    tmp['Sex'] = sex
    df.append(tmp)
df = pd.concat(df)
